...
**Tech stack:** Python 3.12 via `uv` (`uv` is at `/opt/homebrew/bin/uv`; **`mise` is not installed on this Mac** even though `poc-tts/` assumes it — see Global Constraints), `mlx-audio>=0.5.0` (Qwen3-TTS first-class; v0.5.0 released 2026-08-17), `mlx`, `gradio>=5`, `soundfile`, `mlx-whisper` (optional, for auto-transcribing reference clips), `pytest`.
...
| Python | 3.12 via `uv venv` | mlx-audio wheels target 3.10–3.13; 3.12 is present on the machine; 3.14 (Homebrew default) is too new for the mlx/torch ecosystem. |

---

## Global Constraints

- New directory `poc-qwen3-tts/` at repo root, sibling of `poc-tts/`. Do not touch `poc-tts/`, `poc-tts-streaming/`, or `vendor/`.
- Toolchain is **`uv`**, scoped to this directory: `uv venv --python 3.12 .venv` and `uv pip install -r requirements.txt`. No `mise.toml` here; the repo root stays on hermit (`bin/`). Root `Makefile` gets `poc-qwen3-tts-*` targets that delegate with `$(MAKE) -C poc-qwen3-tts`, placed next to the existing `poc-tts-*` block (`Makefile:57`).
...
- Reports go to `poc-qwen3-tts/reports/runs.jsonl` (committed) using the same three bench sentences as `poc-tts/poc_tts/bench.py:39` so rows are comparable with `poc-tts/bench-m4-max.md` (Chatterbox Flash MLX fp16 on this machine: 0.92 s / 1.37 s / 4.20 s short/medium/long, whole-utterance).
...
- Model ids and every knob live in `config.yaml`; env overrides `POC_QWEN3_TTS_<SECTION>_<KEY>` mirror `poc-tts`'s `config.py` pattern.
- Commit after every task on branch `poc-qwen3-tts`.

---

### Task 1: Skeleton, toolchain, smoke test

**Files:** `poc-qwen3-tts/{Makefile,requirements.txt,setup.sh,config.yaml,.gitignore,README.md,poc_qwen3_tts/__init__.py,tests/__init__.py}`; modify root `Makefile`.

- [ ] `setup.sh` (idempotent): fail with a clear message if `uv` missing; `uv venv --python 3.12 .venv` if absent; `uv pip install -r requirements.txt`; write `reports/env_probe.json` with `platform.mac_ver()`, `mlx.__version__`, `mlx_audio.__version__`, `mlx.core.metal.device_info()` (chip name, memory). Never fail setup on the probe.
...
- [ ] `config.yaml`:
  ...
  ```
- [ ] Root `Makefile`: `poc-qwen3-tts`, `poc-qwen3-tts-setup`, `poc-qwen3-tts-bench`, `poc-qwen3-tts-test`, each `@$(MAKE) -C poc-qwen3-tts <target>`; add to `.PHONY` and `help`.
...
**Files:** `poc_qwen3_tts/config.py`, `tests/test_config.py`.

- [ ] Port `poc-tts/poc_tts/config.py` (dataclass sections + `POC_QWEN3_TTS_*` env overrides + type coercion). Reuse its tests' shape (`poc-tts/tests/test_config_overrides.py`).
...
**Files:** `poc_qwen3_tts/engine.py`, `tests/test_engine.py`.
...
**Files:** `poc_qwen3_tts/text.py`, `tests/test_text.py`.
...
**Files:** `poc_qwen3_tts/app.py`, `tests/test_app.py`.
...
- [ ] `python -m poc_qwen3_tts.app` → `demo.queue().launch(server_name=cfg.server.host, server_port=8007)`.
...
**Files:** `poc_qwen3_tts/bench.py`, `tests/test_bench.py`, `reports/runs.jsonl`, `bench-m4-max.md`.
...
**Files:** `poc_qwen3_tts/spike_stream.py`, `reports/stream_spike.jsonl`.
...
- [ ] Commit. Open PR `poc-qwen3-tts` → `main`.
