...
use qwen_tts::engine::{Engine, StreamEvent};
...
impl TtsService for QwenTts {
    ...
    async fn run_tts_stream<'a>(&'a mut self, text: &'a str) -> Result<BoxStream<'a, Frame>> {
        ...
        match rx.recv().await {
            ...
            None => Err(FlowcatError::Other(
                "qwen tts: engine closed the stream".into(),
            )),
        }
    }
}
...
mod tests {
    ...
    async fn events_become_started_twenty_ms_frames_and_stopped() {
        ...
        tx.send(StreamEvent::Audio {
            samples: vec![1; 1000],
        })
        .await
        .unwrap(); // 2 full frames + 40 left
        tx.send(StreamEvent::Audio {
            samples: vec![2; 500],
        })
        .await
        .unwrap(); // 540 → 1 frame + 60
        tx.send(StreamEvent::Done { timings: json!({}) })
            .await
            .unwrap();
        drop(tx);
        ...
        assert_eq!(
            audio,
            vec![480, 480, 480, 60],
            "20 ms frames, remainder flushed on Done"
        );
