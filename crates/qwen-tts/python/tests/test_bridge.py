...
from qwen_tts.bridge import Seam
...
def test_preload_loads_models_and_primes_presets(bridge, fake_loader):
    ...
    # With the test profile's LRU of 2 the Base model is evicted by the third
    # preload and reloaded for voice priming, so look across every instance.
    primed = [c for m in fake_loader.loaded if "1.7B-Base" in m.model_id for c in m.calls if c.get("stream") and c["text"] == "Hi."]
