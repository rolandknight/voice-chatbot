...
from qwen_tts.engine import LANGUAGES, crossfade_concat, discover_voices, lang_key, sidecar_transcript
...
def test_stream_clone_yields_chunks_from_worker(engine, fake_loader):
    ...
    assert len(chunks) == 3 and all(c.dtype == np.float32 for c in chunks)
