...
pub fn make_resampler(
    original_sample_rate: u32,
    model_chunk_size: u32,
    channels_count: usize,
) -> Result<Resamplers, Box<dyn Error>> {
    let sample_rate = original_sample_rate as f32 / MODEL_SAMPLE_RATE as f32;

    if sample_rate.floor() != sample_rate {
        warn!(
            "mic sample rate not supported {}; using double resampling",
            sample_rate
        );
        ...
        let main = Fft::<f32>::new(
            HIGH_SAMPLING_RATE,
            MODEL_SAMPLE_RATE as _,
            main_sampler_input_size,
            1,
            channels_count,
            FixedSync::Input,
        )?;
...
pub fn resample_audio(
    data: &[f32],
    resamplers: &mut Resamplers,
    channels: usize,
) -> Result<Vec<f32>, String> {
    let frames = data.len() / channels;
    let input = InterleavedSlice::new(data, channels, frames)
        .map_err(|e| format!("input adapter error: {}", e))?;

    if resamplers.use_upsampler {
        let presampled = resamplers
            .upsampler
            .process(&input, 0, None)
            .map_err(|e| format!("upsampler error: {}", e))?;
        let resampled = resamplers
            .main
            .process(&presampled, 0, None)
            .map_err(|e| format!("main resampler error: {}", e))?;
