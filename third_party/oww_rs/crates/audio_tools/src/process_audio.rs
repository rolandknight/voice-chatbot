...
pub fn resample_into_chunks(
    ...
    while data_buffer.len() >= chunk_len {
        let resampled_data = if resamplers.resample_rate != 1.0 {
            resample_audio(&data_buffer[..chunk_len], resamplers, channels)
                .expect("resampling error")
        } else {
        ...
        let data_i16: Vec<_> = data_f32
            .iter()
            .map(|c| c.iter().map(f32_to_i16).collect::<Vec<i16>>())
            .collect();
        let rms = data_i16
            .iter()
            .map(|c| calculate_rms(c.as_slice()))
            .max()
            .unwrap_or(0_i16);
...
pub fn split_channels<T: Copy + Clone + Debug>(
    samples: &[T],
    channels: usize,
) -> Box<ChannelsVec<Vec<T>>> {
    match channels {
        1 => {
...
pub fn interlace_stereo(channels: ChannelsData) -> Vec<f32> {
    if channels.iter().len() == 1 {
        ...
        assert_eq!(
            left.len(),
            right.len(),
            "Left and right channels must be of equal length."
        );
