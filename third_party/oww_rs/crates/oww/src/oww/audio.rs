...
impl AudioFeaturesTract {
    pub fn create_default() -> Self {
        ...
        let mut mel_spectrogram_buffer: Box<CircularBuffer<MEL_CIRC_SIZE, Tensor>> =
            CircularBuffer::<MEL_CIRC_SIZE, Tensor>::boxed();
        for _ in 0..MEL_CIRC_SIZE {
            mel_spectrogram_buffer.push_back(
                Tensor::from_shape(&[MELS_PER_CHUNK, 32], &[0f32; MELS_PER_CHUNK * 32]).unwrap(),
            );
        }
    ...
    pub fn get_melspectrogram(&mut self, data: &[f32]) -> Result<Tensor, Box<dyn Error>> {
        ...
        trace!(
            "2: get_melspectrogram with output tensor {:?}",
            out_tensor.shape()
        );
    ...
    pub fn get_audio_features(&mut self, data: &[f32]) -> Result<Tensor, Box<dyn Error>> {
        ...
        self.feature_buffer
            .push_back(embedings[0].clone().into_tensor());
