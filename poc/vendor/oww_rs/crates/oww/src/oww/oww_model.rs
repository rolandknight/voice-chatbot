...
impl OwwModel {
    ...
    pub fn detect(&mut self, features: TractTensor) -> (bool, f32) {
        ...
        if average_detection_probability > self.threshold && since_last_detection > NO_DETECTION_MS as _ {
...
mod poc_probe {
    ...
    fn probe_custom_model_on_wav() {
        let (Ok(model_path), Ok(wav)) = (std::env::var("POC_MODEL"), std::env::var("POC_WAV")) else { return };
        ...
        for chunk in pcm.chunks_exact(1280) {
            ...
            if d.probability > maxp { maxp = d.probability; }
        }
