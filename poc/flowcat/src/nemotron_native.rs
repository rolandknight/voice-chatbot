...
extern "C" {
    ...
    fn nemo_speech_asr_stream_next(stream: *mut NativeStream, out: *mut *mut NativeResult) -> Status;
...
pub fn load_engine(model_path: &str, device: Device, right_context: i32) -> Result<SharedNemotronEngine> {
    ...
    check("create recognizer", unsafe { nemo_speech_asr_create(&cfg, &mut recognizer) })?;
    if recognizer.is_null() {
        return Err(FlowcatError::Other("Nemotron native create returned no recognizer".into()));
    }
    tracing::info!(
...
impl Stream {
    fn create(engine: SharedNemotronEngine, speech_contexts: &[String]) -> Result<Self> {
        ...
        if handle.is_null() {
            return Err(FlowcatError::Other("Nemotron native returned no stream".into()));
        }
        Ok(Self {
            engine,
            handle,
    ...
    fn push(&mut self, samples: &[f32], sample_rate: i32) -> Result<()> {
        ...
        check("push audio", unsafe {
            nemo_speech_asr_stream_push_f32(self.handle, samples.as_ptr(), samples.len(), sample_rate)
        })
    }
    ...
    fn drain(&mut self) -> Result<Vec<Decoded>> {
        ...
        loop {
            ...
            check("stream next", unsafe { nemo_speech_asr_stream_next(self.handle, &mut result) })?;
            if result.is_null() {
                break;
            }
            let decoded = unsafe {
                ...
                let text = if text_ptr.is_null() {
                    ...
                    CStr::from_ptr(text_ptr).to_string_lossy().trim().to_string()
                };
                let d = Decoded {
                    text,
    ...
    fn flush(&mut self) -> Result<Vec<Frame>> {
        ...
        let started = Instant::now();
        check("force endpoint", unsafe { nemo_speech_asr_stream_force_endpoint(self.handle) })?;
    ...
    fn set_muted(&mut self, muted: bool) -> Result<()> {
        ...
        // into the next turn.
        check("force endpoint (mute)", unsafe { nemo_speech_asr_stream_force_endpoint(self.handle) })?;
        let _ = self.drain()?;
        self.hypothesis.clear();
        self.last_interim.clear();
        self.muted = muted;
...
impl Drop for Stream {
    fn drop(&mut self) {
        let _guard = self.engine.lifecycle.lock().unwrap_or_else(|p| p.into_inner());
        unsafe {
...
enum WorkerCommand {
    Audio { samples: Vec<f32>, sample_rate: i32 },
    Flush { reply: oneshot::Sender<Result<Vec<Frame>>> },
    SetMuted { muted: bool, reply: oneshot::Sender<Result<Vec<Frame>>> },
    Shutdown,
}
...
impl SttService for NemotronNativeStt {
    ...
    async fn start(&mut self, _params: &StartParams) -> Result<()> {
        ...
        let worker = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                'worker: loop {
                    ...
                    match command {
                        WorkerCommand::Audio { samples, sample_rate } => {
                            ...
                            loop {
                                match commands_rx.try_recv() {
                                    Ok(WorkerCommand::Audio { samples, sample_rate }) => {
                                        if let Err(e) = stream.push(&samples, sample_rate) {
                                            failed = true;
                                            let _ = updates_tx.send(Err(e));
                                        }
        ...
        match started_rx.await {
            ...
            Err(_) => {
                let _ = worker.join();
                Err(FlowcatError::Other("Nemotron STT worker exited during startup".into()))
            }
        }
    }
    ...
    async fn run_stt(&mut self, audio: Arc<AudioFrame>) -> Result<Vec<Frame>> {
        ...
        let commands = self
            ...
            .ok_or_else(|| FlowcatError::Other("Nemotron STT is not started".into()))?;
        commands
            .send(WorkerCommand::Audio { samples, sample_rate })
            .map_err(|_| FlowcatError::Other("Nemotron STT worker stopped".into()))?;
        self.drain_updates()
    }
    ...
    async fn set_muted(&mut self, muted: bool) {
        ...
        let reset = self.request(|reply| WorkerCommand::SetMuted { muted, reply }).await;
...
fn spoken_text(text: &str) -> Option<String> {
    let text = text.trim();
    text.chars().any(char::is_alphanumeric).then(|| text.to_string())
}
...
mod tests {
    ...
    #[ignore]
    fn native_probe() {
        let poc = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap().to_path_buf();
