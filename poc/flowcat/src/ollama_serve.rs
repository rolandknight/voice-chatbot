...
impl OllamaServe {
    ...
    pub async fn ensure(
        ...
        match plan(supervise, up) {
            ...
            Plan::Spawn => {
                ...
                tracing::info!(elapsed_ms = started.elapsed().as_millis() as u64, "ollama: serve is up");
    ...
    pub fn detach(mut self) {
        if let Some(child) = self.child.take() {
            tracing::info!(pid = child.id(), "ollama: leaving the spawned serve running");
