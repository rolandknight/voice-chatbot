...
impl OllamaLlm {
    ...
    async fn post(&self, path: &str, body: &Value) -> Result<reqwest::Response> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(FlowcatError::Network(format!("ollama {path} {status}: {text}")));
        }
        Ok(resp)
    }
    ...
    pub async fn warm(&self, system_prompt: &str, tools: &[Tool]) -> Result<WarmReport> {
        ...
        Ok(WarmReport {
            ...
            prompt_tokens: v.get("prompt_eval_count").and_then(Value::as_u64).unwrap_or(0),
...
pub fn translate_messages(messages: &[Value]) -> Result<Vec<Value>> {
    ...
    for m in messages {
        ...
        match role {
            ...
            "assistant" => {
                let mut msg = json!({"role": "assistant", "content": content_string(&m["content"])});
...
impl NdjsonState {
    ...
    fn accumulate(&mut self, chunk: &Value) {
        ...
        if let Some(thinking) = msg["thinking"].as_str() {
            if !thinking.is_empty() {
                tracing::warn!(chars = thinking.len(), "ollama: thinking delta despite think=false (dropped)");
            }
        }
    ...
    fn finish(&mut self, done_chunk: Option<&Value>) {
        ...
        if let Some(v) = done_chunk {
            ...
            tracing::info!(
                prompt_tokens, prompt_eval_ms, completion_tokens, eval_ms, ttft_ms,
                load_ms = get("load_duration") / 1_000_000,
                "ollama turn"
            );
            self.pending.push_back(Frame::Metrics(vec![MetricsData::LlmUsage {
...
pub fn ndjson_to_frames<S, B, E>(byte_stream: S, model: String, turn: u64, requested_at: Instant) -> BoxStream<'static, Frame>
where
    ...
    E: std::fmt::Display + Send + 'static,
{
    ...
    futures::stream::unfold((byte_stream.boxed(), state), |(mut bytes, mut st)| async move {
        loop {
...
impl LlmService for OllamaLlm {
    ...
    async fn run_llm<'a>(&'a mut self, ctx: &'a LlmContext) -> Result<BoxStream<'a, Frame>> {
        ...
        let resp = self.post("/api/chat", &body).await?;
        Ok(ndjson_to_frames(resp.bytes_stream(), self.model.clone(), self.turn, requested_at))
    }
...
mod tests {
    ...
    fn request_body_pins_context_and_sorts_tools() {
        ...
        assert_eq!(llm.base_url, "http://127.0.0.1:11434", "/v1 suffix stripped");
    ...
    fn tool_round_trip_translates_openai_shapes() {
        ...
        assert_eq!(out[1]["tool_calls"][0]["function"]["name"], "get_current_time");
        assert_eq!(out[1]["tool_calls"][0]["function"]["arguments"]["tz"], "UTC", "arguments string → object");
        assert!(out[1]["tool_calls"][0].get("id").is_none(), "no ids on the native API");
        assert_eq!(out[2]["role"], "tool");
        assert_eq!(out[2]["tool_name"], "get_current_time", "looked up from the preceding call");
    ...
    async fn stream_error_line_closes_the_response() {
        ...
        .await;
        assert!(matches!(frames.last(), Some(Frame::LlmResponseEnd)));
        assert_eq!(frames.iter().filter(|f| matches!(f, Frame::LlmText(_))).count(), 1);
    }

    #[tokio::test]
    async fn truncated_stream_still_ends() {
        let frames = frames_from(&[r#"{"message":{"role":"assistant","content":"x"},"done":false}"#]).await;
        assert!(matches!(frames.last(), Some(Frame::LlmResponseEnd)));
    }
