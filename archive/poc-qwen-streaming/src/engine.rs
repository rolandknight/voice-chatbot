...
pub enum Cmd {
    ...
    Generate { tab: String, params: Value, tx: mpsc::Sender<StreamEvent> },
}
...
impl Engine {
    ...
    pub fn start(cfg: &Config) -> Result<Self> {
        ...
        ready_rx.recv().context("python thread died during start-up")??;
        Ok(Self { tx })
    }

    fn send(&self, cmd: Cmd) -> Result<()> {
        self.tx.send(cmd).map_err(|_| anyhow!("python thread is gone"))
    }
    ...
    pub fn generate(&self, tab: &str, params: Value) -> Result<mpsc::Receiver<StreamEvent>> {
        ...
        self.send(Cmd::Generate { tab: tab.to_string(), params, tx })?;
        Ok(rx)
    }
}
...
fn python_thread(paths: Vec<PathBuf>, config_path: PathBuf, rx: std_mpsc::Receiver<Cmd>, ready: std_mpsc::Sender<Result<()>>) {
    Python::attach(|py| {
...
fn init_bridge<'py>(py: Python<'py>, paths: &[PathBuf], config_path: &PathBuf) -> Result<Bound<'py, PyAny>> {
    let sys = py.import("sys")?;
    let sys_path = sys.getattr("path")?.cast_into::<PyList>().map_err(|e| anyhow!("{e}"))?;
    ...
    let module = py.import("poc_qwen_streaming.bridge").map_err(|e| {
        ...
        anyhow!("importing poc_qwen_streaming.bridge failed: {e}\n{tb}\nsys.path={:?}", paths)
    })?;
    let bridge = module.getattr("Bridge")?.call1((config_path.to_string_lossy().as_ref(),))?;
    tracing::info!("python bridge ready ({})", sys.getattr("version")?.extract::<String>()?.lines().next().unwrap_or(""));
    Ok(bridge)
}
...
fn handle(py: Python<'_>, bridge: &Bound<'_, PyAny>, cmd: Cmd) {
    match cmd {
        Cmd::Info(reply) => {
            let _ = reply.send(bridge.call_method0("model_info").and_then(|o| to_json(py, &o)).map_err(pyerr(py)));
        }
        Cmd::Catalog(reply) => {
            let _ = reply.send(catalog(py, bridge).map_err(pyerr(py)));
        }
        Cmd::VoicePath(name, reply) => {
            let _ = reply.send(bridge.call_method1("voice_path", (name,)).and_then(|o| o.extract::<Option<String>>()).map_err(pyerr(py)));
        }
        Cmd::Transcribe(path, reply) => {
            let _ = reply.send(bridge.call_method1("transcribe", (path,)).and_then(|o| o.extract::<String>()).map_err(pyerr(py)));
        }
        Cmd::Unload(reply) => {
        ...
        Cmd::Preload(reply) => {
            let r = bridge.call_method0("preload").and_then(|o| to_json(py, &o)).map_err(pyerr(py));
            match &r {
...
fn json_to_py<'py>(py: Python<'py>, v: &Value) -> PyResult<Bound<'py, PyAny>> {
    Ok(match v {
        ...
        Value::Array(a) => {
            let items: Vec<Bound<'py, PyAny>> = a.iter().map(|x| json_to_py(py, x)).collect::<PyResult<_>>()?;
...
fn generate(py: Python<'_>, bridge: &Bound<'_, PyAny>, tab: &str, params: &Value, tx: &mpsc::Sender<StreamEvent>) -> Result<()> {
    let t0 = Instant::now();
    ...
    let model: String = bridge
        ...
        .unwrap_or_default();
    let iter = bridge.call_method1("stream", (tab, &py_params, &stop)).map_err(pyerr(py))?;
    ...
    loop {
        ...
        let arr = audio.extract::<numpy::PyReadonlyArray1<f32>>().map_err(|e| anyhow!("chunk is not a float32 array: {e}"))?;
        ...
        if !started {
            ...
            if tx.blocking_send(StreamEvent::Start { sample_rate: sr, model: model.clone() }).is_err() {
                cancelled = true;
            }
        }
        total_samples += samples.len();
        chunks += 1;
        if !cancelled && py.detach(|| tx.blocking_send(StreamEvent::Audio { samples })).is_err() {
            cancelled = true;
        }
        if cancelled {
