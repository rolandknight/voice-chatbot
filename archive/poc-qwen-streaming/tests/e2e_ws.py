...
voice over /ws exactly as the browser does, and report client-side TTFA.
    ...
    ../poc-qwen/.venv/bin/python tests/e2e_ws.py [--size 0.6B] [--text "..."]
...
BIN = HERE / "target" / "release" / "poc-qwen-streaming"
...
async def main() -> int:
    ...
    proc = subprocess.Popen([str(BIN), "--config", str(HERE / "config.yaml"), "serve"], cwd=HERE, env=env)
