.PHONY: poc-setup poc-build poc-up poc-down poc-test poc-test-all poc-results help install-server install-server-os install-client install-client-os run run-webrtc-smoke run-webrtc-smoke-lan run-server run-server-lan run-server-local run-server-lan-local run-webrtc-client run-rpi-client-local run-jabra run-wake-test run-wake-client
