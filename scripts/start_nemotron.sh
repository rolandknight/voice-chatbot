...
HOST_OVERRIDE_SET="${NEMOTRON_HOST+set}"
HOST_OVERRIDE="${NEMOTRON_HOST-}"
PORT_OVERRIDE_SET="${NEMOTRON_PORT+set}"
PORT_OVERRIDE="${NEMOTRON_PORT-}"
...
[ -n "$HOST_OVERRIDE_SET" ] && NEMOTRON_HOST="$HOST_OVERRIDE"
[ -n "$PORT_OVERRIDE_SET" ] && NEMOTRON_PORT="$PORT_OVERRIDE"
[ -n "$MODEL_ROOT_OVERRIDE_SET" ] && NEMO_SPEECH_MODEL_DIR="$MODEL_ROOT_OVERRIDE"

NATIVE_ROOT="$DIR/.deps/nemo-speech/v0.1.0"
NEMO_SPEECH_BIN="$NATIVE_ROOT/bin/nemo-speech"
MODEL_ROOT="${NEMO_SPEECH_MODEL_DIR:-$DIR/models/nemotron}"
DEVICE="${NEMOTRON_DEVICE:-auto}"
HOST="${NEMOTRON_HOST:-127.0.0.1}"
PORT="${NEMOTRON_PORT:-8178}"
RIGHT_CONTEXT="${NEMOTRON_RIGHT_CONTEXT:-6}"
...
*) fail "invalid NEMOTRON_DEVICE '$DEVICE' (expected auto, cuda:0, cpu, or metal)" ;;
esac

case "${PLATFORM_OVERRIDE:-$(uname -s)}" in
Linux)
    [ "$DEVICE" != "metal" ] || fail "NEMOTRON_DEVICE=metal is only supported on macOS"
    ;;
Darwin)
    [ "$DEVICE" != "cuda:0" ] || fail "NEMOTRON_DEVICE=cuda:0 is only supported on Linux"
    if [ "$(uname -m)" = "x86_64" ] && [ "$DEVICE" = "metal" ]; then
        fail "NEMOTRON_DEVICE=metal requires Apple Silicon"
    fi
    ;;
...
case "$PORT" in
'' | *[!0-9]*) fail "NEMOTRON_PORT must be an integer from 1 through 65535" ;;
esac
[ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ] || \
    fail "NEMOTRON_PORT must be an integer from 1 through 65535"
...
0 | 1 | 6 | 13 | -1) ;;
*) fail "NEMOTRON_RIGHT_CONTEXT must be one of 0, 1, 6, 13, or -1" ;;
esac
